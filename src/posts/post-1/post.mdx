...
knowledge, has come from looking stuff up online. I'll rant more about this in a [later post](https://rmaa.dev/posts/productivity).

So yeah, **I could've used a template**, I could've forked some Github project and just changed a few lines
and called it a day. I was actually considering using Jekyll, a Ruby based framework.
I could've used wordpress. My site wouldn't look nearly as bad. But that wouldn't have been fun, and coding
should be fun! &#128521;

Through trying to make this site, I obviously drew some inspiration on how other people made their sites.
And I realized that some people really know how to make websites! I mean, just look
at [this one](https://youtu.be/BZqzhmlTkAc?t=200). Can you even call that a personal site anymore
&#128531;
...
tool that I'll perhaps encounter someday.
...
2.  **GraphQL** is super cool
    ...
    - Here's another example of a query. To make it interesting, let's look at some source code from my 
    website!
    To load up all the projects in the [projects page](https://rmaa.dev/projects), I first make a **query** to the 
    CMS...
    ...
    - To break it down, I get all the projects from the property called *allContentfulProject* which 
    is the name of the graphql collection. I sort it by the name alphabetically.

    - I access the *nodes* property which is kind of like the content I suppose. Then I grab
    the **image**, the **projectName**, the **link**, the **shortDescription**, etc.

    - Now the data will all come in as a props called "data". So I can simply dereference it to what I want:
    ...
    ```js
      ...
      // more code here... Standard react functional component with return render function
      return (
    ...
    So this looks a bit confusing but all that's being done is, we're looping through every project 
    and then **accessing every property that we fetched for earlier**. So now we can simply just 
    use them when rendering different HTML elements. For example, when rendering the h5 element I just 
    put "{projectName}" which came from **the graphql query**
    ...
    - In summary, graphQL is super powerful and I am glad I learned it. It really isn't too
      ...
      backend (where all the assets and data are) and makes for a much nicer development experience.
...
6.  **Styled Components** are super useful because you don't have to worry about colliding class names
    ...
    They are pretty easy to use and make.
...
So in this example- if we had a class in our CSS files called 'category', well, we don't need to worry
about naming it something different in our styled components because it sits insde of the
component and will be **renamed to something entirely different** once it does its magic! Classnames
stay local to that component + its children so its a lot easier to manage.

7.  **SASS** is the best invention ever and should honestly be in default CSS

    First off, thank god I can comment:
    ...
    Also, SASS variables are awesome, and allow for even more customizability. Just have a file 
    called "variables.scss" and then put all your colors in it like this:
    ...
    then whenevery you want to use a color just do 
    ...
    I know this is in vanilla CSS as well though, so this isn't SASS specific.

    
    100% the best part about SASS is the nested CSS features. Say you want to style 
    a bunch of stuff in a div element.
    ...
    Like it's just no contest. Especially once your divs start getting bloated
    with a lot of elements, it gets really 
    confusing really fast. 
    
    Okay, I'm done raving about SASS/SCSS, but needless to say I will 
    be using it from now on. It's just better CSS in my opinion.
...
10. **Commentbox** for comments!
    ...
    a quick search and a few commands. Look forward to interacting with people :)

    <hr />

11. *Prism* for syntax highlighting! 

    I'm bound to showcase a bunch of code I find interesting on this blog, so I gotta make it look nice!
    I used the [prism-react-renderer](https://www.npmjs.com/package/prism-react-renderer) package to do this.
    But some of the languages I want are **not supported** in this version so you may have to install 
    the main prism package as well using 
